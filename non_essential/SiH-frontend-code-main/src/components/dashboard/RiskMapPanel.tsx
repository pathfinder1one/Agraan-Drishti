...
export function RiskMapPanel({ heatmapData, onCellClick, selectedCell, monitoredLocation, activeLayer, onLayerChange }: RiskMapPanelProps) {
  ...
  const [selectedId, setSelectedId] = useState<string | null>("rudraprayag");

  const selected = MAP_MARKERS.find((m) => m.id === selectedId) ?? null;
        ...
        <div className="relative flex-1 min-h-[480px] bg-[#0b0f18] overflow-hidden rounded-br-xl">
          ...
          {MAP_MARKERS.map((m) => (
            <button
              ...
              aria-label={`Risk marker: ${m.name}`}
            >
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className={`block rounded-full border-2 ${
                  m.level === "extreme" ? "animate-pulseRing" : ""
                }`}
                style={{
                  width: selectedId === m.id ? 15 : 11,
                  height: selectedId === m.id ? 15 : 11,
                  background: LEVEL_COLOR[m.level],
                  borderColor: selectedId === m.id ? "#fff" : "#0b0f18",
                }}
              />
            </button>
          ))}

          <div className="absolute top-16 left-1/2 -translate-x-1/2 text-[11.5px] font-semibold text-slate-200 pointer-events-none">
            Rudraprayag
          </div>
          ...
          <AnimatePresence>
            {selected && (
              <motion.div
                ...
                className="absolute right-4 top-4 w-64 rounded-xl border border-border bg-panel shadow-2xl overflow-hidden"
              >
                ...
                <div className="px-3 pt-2 pb-3 space-y-2">
                  <Badge level={selected.level}>
                    {selected.level === "extreme" ? "Extreme risk" : selected.level === "high" ? "High risk" : "Moderate risk"}
                  </Badge>
                  {[
                    ["Flash flood probability", `${selected.flashFlood}%`],
                    ["Cloudburst probability", `${selected.cloudburst}%`],
                    ["Thunderstorm probability", `${selected.thunderstorm}%`],
                    ["ETA to impact", selected.eta],
