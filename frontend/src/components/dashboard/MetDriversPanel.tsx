...
import { Wind, ArrowUpRight, ArrowDownRight, Minus, Activity } from "lucide-react";
...
export function MetDriversPanel({ xaiData, selectedCell, monitoredLocation, maxRisks, realtimeWeather }: MetDriversPanelProps) {
  ...
  useEffect(() => {
    if (xaiData) {
      ...
      apiFetch(`/api/xai/${lat.toFixed(2)}/${lon.toFixed(2)}`)
        .then(res => res.json())
  ...
  const dynamicDrivers = [
    {
      ...
      color: cape > 2000 ? "text-destructive" : (cape > 1000 ? "text-amber-700 dark:text-amber-400" : "text-emerald-700 dark:text-emerald-400")
    },
    {
      ...
      color: humidity > 85 ? "text-accent" : "text-emerald-700 dark:text-emerald-400"
    },
    {
      ...
      color: pressure < 1005 ? "text-amber-700 dark:text-amber-400" : "text-ink"
    },
    {
      ...
      color: Math.abs(cin) < 40 ? "text-amber-700 dark:text-amber-400" : "text-emerald-700 dark:text-emerald-400"
    },
    {
      ...
      color: iwv > 3.0 ? "text-destructive" : "text-accent"
    },
    {
      ...
      color: shear > 10 ? "text-amber-700 dark:text-amber-400" : "text-ink"
    },
    {
      ...
      color: precip > 15 ? "text-destructive" : (precip > 2 ? "text-accent" : "text-emerald-700 dark:text-emerald-400")
    },
    {
      ...
      color: soilSaturation > 80 ? "text-destructive" : "text-emerald-700 dark:text-emerald-400"
    },
    {
      ...
      color: ctt < -60 ? "text-destructive" : "text-accent"
    }
  ];
  ...
  return (
    <BentoCard glowBorder="accent" className="flex flex-col h-full">
      <CardHeader 
        icon={Wind} 
        title="Meteorological Drivers" 
        subtitle="Physical Atmosphere Thermodynamic Sounding"
        right={
          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-secondary text-accent border border-border font-bold flex items-center gap-1.5 shadow-2xs">
      ...
      <CardBody className="flex-1 p-3.5 space-y-2 flex flex-col justify-between">
        <div className="space-y-1.5 overflow-y-auto max-h-[185px] pr-1">
          {dynamicDrivers.map((d) => (
            <div key={d.label} className="flex items-center justify-between text-[11px] py-1 border-b border-border/50 last:border-b-0">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                {d.trend === "up" && <ArrowUpRight size={12} className="text-destructive" />}
                {d.trend === "down" && <ArrowDownRight size={12} className="text-accent" />}
        ...
        <div className="pt-2 border-t border-border/60 flex items-center justify-between text-[10px] text-ink-faint font-mono">
          <span>Coordinates: {lat.toFixed(2)}°N, {lon.toFixed(2)}°E</span>
          <span className="text-accent flex items-center gap-1 font-bold">
            <Activity size={11} /> Dynamic XAI
          </span>
        </div>
      </CardBody>
    </BentoCard>
